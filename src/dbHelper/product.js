...
class productService {
    ...
    searchByName(keyWord) {
        const arrayKeyWords = keyWord.split(" ").map(word => `"${word}"`).join(' ');
        return product.find({ $text: { $search: arrayKeyWords } });

    }

    getProductsByCategory(slug) {
        try {
            return product.find({ nameCategory: slug }, { name: 1, price: 1, image: 1, _id: 1 });
        } catch (error) {
